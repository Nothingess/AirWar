...
export function gen_handler(cb: Function, host: any = null, ...args: any[]): handler {
    let single_handler: handler = null;
    if (handler_pool.length <= 0) {
        single_handler = new handler();
    } else {
        single_handler = handler_pool.pop();
    }
...
export class LoaderMgr {
     ...
     */
    loadResArray(urls: string[], cb: handler, pro?: (completedCount: number, totalCount: number, item: any) => void, assetTypes?: typeof cc.Asset[], alias?: string[]) {
        //加载同名资源时，需要手动给出不同的别名
        let loaded_res = {};
        let unloaded_urls: string[] = [];
        ...
        cc.loader.loadResArray(unloaded_urls, pro, err => {
            if (err) {
